...
**Символьные устройства (char devices)** — передают данные *потоком байтов*, без произвольного доступа по смещению.  
Примеры:  
- `/dev/tty` (терминал)  
- `/dev/random`  

**Блочные устройства (block devices)** — работают с *блоками фиксированного размера* и поддерживают произвольный доступ.  
Примеры:  
...
`busy-wait` постоянно крутит цикл и проверяет состояние ресурсов, **сжигая 100% CPU времени**, даже если данных нет.

---

`poll()` передаёт управление ядру и блокируется до появления события, освобождая процессор другим задачам.

**Критический сценарий:**  
Сервер, обслуживающий 1000+ сетевых соединений или датчиков.  
`busy-wait` мгновенно загрузит ядро на 100%, а `poll()` позволит эффективно ждать события от многих источников.
...
Да. Многие символьные устройства (например, `/dev/tty`, `/dev/input/event*`) блокируют `read()`, если **нет данных**.

Избежать блокировки можно так:
- открыть устройство с флагом **`O_NONBLOCK`**  
- или использовать **`select/poll/epoll`** для ожидания данных  
- или применить **таймауты** (например, `termios`)
...
Файлы в `/dev/input/` используют *Linux Input Subsystem*, а `ioctl` коды и структуры (например, `input_event`, `EV_KEY`, `EV_REL`) определены только в Linux-заголовках:  
- `<linux/input.h>`  
- `<linux/ioctl.h>`  

Эти интерфейсы **не стандартизированы POSIX** и отличаются на macOS, FreeBSD и других системах.