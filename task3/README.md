...
**POSIX Message Queue** — подходит, когда нужно передавать небольшие сообщения фиксированного формата между процессами на одной машине, с возможностью приоритизации.  
**Пример:** процесс-производитель отправляет события процессу-обработчику, где важен приоритет сообщений.

**UNIX Sockets** — лучше, когда требуется потоковая передача, переменный размер данных, двусторонняя связь или совместимость с сетевой моделью.  
**Пример:** локальный клиент и сервер обмениваются запросами и ответами, как по TCP, но через файловый путь.
...
`epoll` сообщает о готовности каждый раз, пока данные остаются в буфере. Простой в использовании.

`epoll` сообщает только при **изменении состояния** (появились данные). Если не вычитать буфер до конца — событие больше не придёт.  
**Сложнее**, потому что:  
- нужно читать/писать **до EAGAIN**,  
- легко пропустить событие, если не опустошить буфер,  
- ошибки часто связаны с “залипшими” дескрипторами, которые больше не генерируют события.
...
*Обычный `pthread_mutex_t` нельзя использовать между разными процессами**, он работает только внутри одного процесса.

Можно использовать **процесс-shared мьютекс**, если создать его в *shared memory* и установить атрибут:  
- `pthread_mutexattr_setpshared(&attr, PTHREAD_PROCESS_SHARED)`  

Тогда мьютекс можно шарить между процессами, но он работает только в общей памяти (shm).
...
От большего количества копирований к меньшему:

a. **UNIX Sockets** — 2 копирования:  
   user → kernel, kernel → user.
b. **POSIX Message Queue** — обычно тоже 2 копирования (в ядро и обратно), но через свои внутренние буферы.
c. **Shared Memory** — **0 копирований**: процессы работают с одним и тем же участком памяти, все данные общие.

Поэтому shared memory — самый быстрый механизм передачи.